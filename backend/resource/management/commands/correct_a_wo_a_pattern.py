...
from resource.models import Attendance, Employee, AWOWorkOffCorrection
...
class Command(BaseCommand):
    ...
    For records up to today from the earliest attendance record date in the database.
    ...
    def handle(self, *args, **options):
        ...
        Retrieves attendance records from the earliest date to today, identifies 'A-WO-A' patterns,
        ...
        self.stdout.write(self.style.SUCCESS(
            f"Starting A-WO-A pattern check from earliest date: {earliest_logdate} to {today}"
        ))

        # Calculate total number of days to process for TQDM progress bar
        total_days = (today - earliest_logdate).days + 1

        # Iterate through dates from the earliest log date to today, using tqdm for progress bar
        with tqdm(total=total_days, desc="Processing Dates", unit="day", ncols=80) as pbar:
            date_iterator = self.date_range_generator(earliest_logdate, today)
            for day1, day2, day3 in date_iterator:
                ...
                for employee_id in employee_attendance_map:
                    ...
                    if day1 in employee_dates_attendance and day2 in employee_dates_attendance and day3 in employee_dates_attendance:
                        ...
                        if (day1_record.shift_status == 'A' and day2_record.shift_status == 'WO' and day3_record.shift_status == 'A'):
                            ...
                            # Create AWOWorkOffCorrection record
                            with transaction.atomic():
                                _, created = AWOWorkOffCorrection.objects.get_or_create(
                                    employee=day2_record.employeeid,
                                    corrected_date=day2,
                                    defaults={
                                        'day1_date': day1,
                                        'day3_date': day3,
                                ...
                                if not created:
                                    self.stdout.write(self.style.WARNING(
                                        f"Duplicate correction record found for employee {day2_record.employeeid} on {day2}."
                                    ))
        ...
        self.stdout.write(self.style.SUCCESS("Successfully processed employees from earliest date to today."))
